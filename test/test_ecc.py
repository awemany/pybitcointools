...
class TestSigningProcess(unittest.TestCase):
    # These test vectors are taken from the following Stack Overflow topic:
    # https://bitcoin.stackexchange.com/questions/50980/test-r-s-values-for-signature-generation
    # Their original source appears to be a bitcointalk thread:
    ...
    for tv in test_vectors:
        priv, msg, k, rs = tv
        msghash = sha256(msg)
        assert k == deterministic_generate_k(msghash, priv)

        v, r, s = ecdsa_raw_sign(msghash, priv)
        assert encode(r, 16, 64)+encode(s, 16, 64) ==  rs
